          ...
          if (always || prev.init || current.init) {
            context.report({
              ...
              fix: (fixer) => {
                ...
                if (tokenBefore) {
                  const range = [tokenBefore.range[1], current.range[0]] as const
                  const betweenText = sourceCode.getText().slice(range[0], range[1])

                  return fixer.replaceTextRange(range, `${betweenText}\n`)
                }
                else {
